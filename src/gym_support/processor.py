...
The trace shape in LangSmith:
    ...
    └── turn × N                      (per exchange; carries was_interrupted)
        ...
        │   ├── model                 (model inference; may emit tool calls)
        │   ├── tools: check_credits  (tool / handoff execution)
...
class LangSmithSpanProcessor(SpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        # Nested LangGraph/LangChain runs (the model node, its chat-model calls,
        # tools/handoffs) need no rewriting — they arrive in LangSmith's
    ...
    def _exclude_from_message_view(self, span: ReadableSpan) -> None:
        ...
        its chat-model calls, plus the gym tools — form the clean
...
def setup_langsmith_tracing(
    *,
    ...
    `OTEL_EXPORTER_OTLP_HEADERS` from the environment (set in `.env`). Returns
    the processor instance so the agent can register its audio recorders on it.
    ...
    setup_tracing(service_name="gym-support-voice", exporter=None, console_export=False)
